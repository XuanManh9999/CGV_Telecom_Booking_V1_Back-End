...
async def create_type_number(type_number : TypeNumberCreate , db: AsyncSession = Depends(get_db)):
   return await handle_type_number.create_type_number(db=db, type_number_client=type_number)


@router.put("/type-number-by-id", response_model=TypeNumberUpdate)
async def update_type_number_by_id(type_number_id : int, type_number : TypeNumberUpdate , db: AsyncSession = Depends(get_db)):
   return await handle_type_number.update_type_number_by_id(type_number_id, type_number_client=type_number, db=db)

@router.delete("/{type_number_id }")
async def delete_type_number(id_type_number: int, db: AsyncSession = Depends(get_db)):
   return await handle_type_number.delete_type_number_by_id(type_number_id=id_type_number, db=db)